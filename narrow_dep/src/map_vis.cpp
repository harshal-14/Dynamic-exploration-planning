...
////edges
//////////////
ros::Publisher edge_vis_pub ;// Publisher for edges ....
ros::Publisher closebound_vis_pub; ///Publisher for boundary line of sampling
ros::Publisher inner_map_vis_pub;
//////////////
...
//////////////////////narrows //////////////////////////////////////////////////////narrowssss...new

PRM* inner_roadmap;
int inner_map_size=0;
int closed_fail_count=0;
Node* current_return_point;
Node* prior_entry_node;
visualization_msgs::MarkerArray edge_markers;
std::vector<visualization_msgs::Marker> edge_vis_array;

visualization_msgs::MarkerArray closebound_markers;
std::vector<visualization_msgs::Marker> closebound_vis_array;

visualization_msgs::MarkerArray inner_map_markers;
std::vector<visualization_msgs::Marker> inner_map_vis_array;

bool going_for_search;
bool back_to_roadmap;
std::vector<Node*>return_goal;
/////////////////////////
visualization_msgs::Marker path_marker;
visualization_msgs::Marker old_path_marker;
std::vector<Node*> path;
std::vector<Node*> old_path;
int closed_search_counter;
int path_idx = 0;
DEP::Goal next_goal;
...
void callback(const nav_msgs::OdometryConstPtr& odom, const octomap_msgs::Octomap::ConstPtr& bmap){
	...
	if (reach){
		...
		if (path_idx >= path.size()){
			...
			roadmap->closed_search=true;
			std::cout<<"I m at 165"<<std::endl;
			/////current_return_point->num_voxels=0.5*(current_return_point->num_voxels); 1 change here 
			current_return_point->num_voxels=0;
			current_return_point->entry_explored=true;
			//inner_roadmap->insert(current_return_point);
			std::cout<<"address of current_return_point"<<current_return_point<<std::endl;
			inner_roadmap->insert(current_return_point);
			std::cout<<"address of current_return_point -2"<<current_return_point<<std::endl;
			std::cout<<"I m at 167"<<std::endl;
				for(Node* n_:current_return_point->adjNodes){

				n_->entry_explored=true;
				n_->num_voxels=0;////////////2 change here ///all neigbhors in outer map explored and 0s 
			
				}
			closed_fail_count=0;
			going_for_search=false;
			back_to_roadmap=false;}

			if(back_to_roadmap){
			//return_goal.clear();
			roadmap->closed_search=false;
			closed_fail_count=0;
			going_for_search=false;
			back_to_roadmap=false;
			current_return_point->entry_explored=true;
			for(Node* n:inner_roadmap->getRecord()){
				if(n!=current_return_point){
					n->came_from_innermap=true;
					double x=n->p.x();
		            double y=n->p.y();
		            double z=n->p.z();
			...
			inner_roadmap->clear();
			for(Node* n_:current_return_point->adjNodes){
				if(n_->iam_entry==true){
				n_->entry_explored=true;
				for (Node* nn_:n_->adjNodes){
					if(nn_->iam_entry==true){
						nn_->entry_explored=true;
					}
				}
			}
			}
	...
	if (new_plan){
		...
		if (first_time){
			roadmap = new PRM ();
			inner_roadmap=new PRM ();	
		}
		cout << "==============================" << count_iteration << "==============================" << endl;
		
		if (first_time){
			roadmap->closed_search=false;
			closed_search_counter=0;
			roadmap = buildRoadMap(*tree_ptr, roadmap, path,  NULL, map_vis_array,edge_vis_array,inner_map_vis_array,roadmap->closed_search,NULL,NULL,NULL);
			start = findStartNode(roadmap, &current_pose, *tree_ptr);
			first_time = false;
			
			// cout << "here3" <<endl;
			goal_candidates = getGoalCandidates(roadmap);
		...
		// cout << "here5" <<endl;
			...
			start = last_goal;	
			if(!roadmap->closed_search){
			closed_search_counter=0;
			roadmap = buildRoadMap(*tree_ptr, roadmap, path,  start, map_vis_array,edge_vis_array,inner_map_vis_array,roadmap->closed_search,NULL,NULL,NULL);
			// cout << "here1" <<endl;
		...
		path = findBestPath(roadmap, start, goal_candidates, *tree_ptr, replan);
		////before this it was working /// bad narrows
		if(going_for_search && path.size()==0){
			roadmap->clearEntryGoals();
			goal_candidates = getGoalCandidates(roadmap);
			path = findBestPath(roadmap, start, goal_candidates, *tree_ptr, replan);
		...
		++closed_search_counter;
		///////////////working as normal PRM inside closed space///
		inner_roadmap = buildRoadMap(*tree_ptr, inner_roadmap, path,  start, map_vis_array,edge_vis_array,inner_map_vis_array,roadmap->closed_search,roadmap,current_return_point,prior_entry_node);
			// cout << "here1" <<endl;
		if (inner_roadmap->getSize() - inner_map_size < 3){
		...
		if (closed_fail_count >= 5){
			
			goal_candidates.push_back(current_return_point);
			
			back_to_roadmap=true;
			auto start_time_search = high_resolution_clock::now();
		   // cout << "here4" <<endl;
		   path = findBestPath(inner_roadmap, start, goal_candidates, *tree_ptr, replan);
		...
		inner_map_size = inner_roadmap->getSize();

		//auto start_time_search = high_resolution_clock::now();
		std::vector<Node*> goal_candidates;
		// cout << "here3" <<endl;
		if(inner_map_size>1){
		goal_candidates= getGoalCandidates(inner_roadmap);
		...
		if (path.size() != 0){
			if(going_for_search){
				if(path.back()!=last_goal){
					path.push_back(last_goal);
				}
				std::cout<<"Address before"<<current_return_point<<std::endl;
				std::cout<<"Address of path"<<path.back()<<std::endl;
				current_return_point=path.back();
                std::cout<<"Address after"<<current_return_point<<std::endl;
				prior_entry_node=path[path.size()-3];
			}
			next_goal = getNextGoal(path, path_idx, odom);
			reach = false;
			++path_idx;
		...
		edge_markers.markers=edge_vis_array;
		inner_map_markers.markers = inner_map_vis_array;
		if (path.size() != 0){
			...
			if(going_for_search || roadmap->closed_search){
				closebound_vis_array.clear();
				std::vector<geometry_msgs::Point> node_vis_array;
				for (point3d p: current_return_point->wallpoints){
		        ...
		        node_vis_marker.color.r = 1.0;
		        node_vis_marker.color.g = 1.0;
		        node_vis_marker.color.b = 1.0;
		// cout << "NODE VIS ARRAY SIZE: "<< node_vis_array.size() << endl;
		       closebound_vis_array.push_back(node_vis_marker);


			closebound_markers.markers = closebound_vis_array;}
		...
		path = findBestPath(roadmap, start, goal_candidates, *tree_ptr, replan);}
			...
			std::vector<Node*> goal_candidates = getGoalCandidates(inner_roadmap);
			//////////////narrowss////////////////////////////////////////////////////
			Node* start = findStartNode(inner_roadmap, &current_pose, *tree_ptr); 
...
int main(int argc, char** argv){
	...
	edge_vis_pub=nh.advertise<visualization_msgs::MarkerArray>("edge_vis_array",0) ; 
	closebound_vis_pub=nh.advertise<visualization_msgs::MarkerArray>("closebound_vis_array",0) ; 
	inner_map_vis_pub = nh.advertise<visualization_msgs::MarkerArray>("inner_map_vis_array", 0);
	...
	while (ros::ok()){
		...
		edge_vis_pub.publish(edge_markers);
		closebound_vis_pub.publish(closebound_markers);
		inner_map_vis_pub.publish(inner_map_markers);
...
std::vector<Node*> getGoalCandidates(PRM* roadmap){
	...
	std::priority_queue<Node*, std::vector<Node*>, GainCompareNode> entry_goals = roadmap->getEntryGoals();
	while (max_number<5){
		if(entry_goals.empty()){
			break;
		}
		Node* n = entry_goals.top();
		entry_goals.pop();
		goal_candidates.push_back(n);
	}
	if(goal_candidates.size()>0){
	going_for_search=true;
	...
	int size_goal_vector=goal_nodes.size();
	int iter_count=1;
	while (true){
		...
		if (iter_count==size_goal_vector){
			break;
		}
